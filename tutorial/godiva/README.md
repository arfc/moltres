...
To run these input files and subsequent group constant generation scripts, the user must install OpenMC version 0.13.2 or later (https://github.com/openmc-dev/openmc/).
...
The only notable difference is in the FISSE group constant value. OpenMC outputs FISSE values of ~193 MeV, while Serpent outputs FISSE values of ~202 MeV. A possible reason for this discrepancy is that Serpent includes neutrino energies while OpenMC does not. 
