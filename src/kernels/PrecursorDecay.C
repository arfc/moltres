...
{
  return _test[_i][_qp] * _decay_constant[_qp][_precursor_group] * computeConcentration();
}

Real
PrecursorDecay::computeQpJacobian()
{
  return _test[_i][_qp] * _decay_constant[_qp][_precursor_group] * computeConcentrationDerivative();
}
...
{
  if (jvar == _temp_id)
    return _test[_i][_qp] * _d_decay_constant_d_temp[_qp][_precursor_group] * _phi[_j][_qp] * computeConcentration();
