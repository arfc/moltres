...
{
  ADRealTensorValue vorticity = _grad_velocity[_qp] - _grad_velocity[_qp].transpose();
...
//  ADReal ft2 = _ct3 * std::exp(-_ct4 * chi * chi);
  ...
  ADReal source_part = -(1) * _rho[_qp] * _cb1 * std::max(S, 0.) * _u[_qp] * _test[_i][_qp];
...
//  ADReal psi_supg = _taum[_qp] * _vel_elem * _grad_test[_i][_qp];
