#include "PrecursorEigenSource.h"

registerMooseObject("MoltresApp", PrecursorEigenSource);

InputParameters
PrecursorEigenSource::validParams()
{
  ...
  params.addParam<Real>("prec_scale", 1, "Thefactor by which the neutron fluxes are scaled.");
  return params;
}

PrecursorEigenSource::PrecursorEigenSource(const InputParameters & parameters)
...
Real
PrecursorEigenSource::computeQpResidual()
{
  Real r = 0;
...
Real
PrecursorEigenSource::computeQpJacobian()
{
  return 0;
}

Real
PrecursorEigenSource::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real jac = 0;
  for (unsigned int i = 0; i < _num_groups; ++i)
    if (jvar == _flux_ids[i])
