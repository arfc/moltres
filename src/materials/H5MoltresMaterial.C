...
H5MoltresMaterial::dummyComputeQpProperties()
{
  ...
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _d_remxs_d_temp[_qp][i] =
        _xsec_linear_interpolators["REMXS"][i].sampleDerivative(dummy_temp);
    _d_fissxs_d_temp[_qp][i] =
        _xsec_linear_interpolators["FISSXS"][i].sampleDerivative(dummy_temp);
    _d_nsf_d_temp[_qp][i] =
        _xsec_linear_interpolators["NSF"][i].sampleDerivative(dummy_temp);
    _d_fisse_d_temp[_qp][i] =
        _xsec_linear_interpolators["FISSE"][i].sampleDerivative(dummy_temp) * 1e6 *
        1.6e-19; // convert from MeV to Joules
    ...
    _d_chi_d_temp[_qp][i] =
        _xsec_linear_interpolators["CHI"][i].sampleDerivative(dummy_temp);
    _d_chi_d_d_temp[_qp][i] =
        _xsec_linear_interpolators["CHI_D"][i].sampleDerivative(dummy_temp);
  }
  ...
  for (decltype(_num_groups) i = 0; i < _num_precursor_groups; ++i)
  {
    ...
    _decay_constant[_qp][i] =
        _xsec_linear_interpolators["DECAY_CONSTANT"][i].sample(dummy_temp);
...
H5MoltresMaterial::monotoneCubicComputeQpProperties()
{
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _fisse[_qp][i] = _xsec_monotone_cubic_interpolators["FISSE"][i].sample(_temperature[_qp]) * 1e6 *
                     1.6e-19; // convert from MeV to Joules
  ...
  for (decltype(_num_groups) i = 0; i < _num_groups * _num_groups; ++i)
  {
    _gtransfxs[_qp][i] = _xsec_monotone_cubic_interpolators["GTRANSFXS"][i].sample(_temperature[_qp]);
    _d_gtransfxs_d_temp[_qp][i] =
