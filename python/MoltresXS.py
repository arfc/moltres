        ...
        for i in range(num_burn):
            ...
            for j in range(num_uni):
                ...
                for k in range(num_temps):
                    ...
                    self.xs_lib[i][j][k]["FISSE"]   = np.divide(fissE[index:index+num_groups],fissxs[index:index+num_groups],out=np.zeros_like(fissE[index:index+num_groups]), where=fissxs[index:index+num_groups]!=0)
    ...
    while k < len(lines):
        ...
        if '[BRANCH]' in line:
            ...
            for i in range(tot_branch):
                ...
                mat_dict[val[0]]['temps'] = np.append(int(val[1]),mat_dict[val[0]]['temps'])
                mat_dict[val[0]]['file']  = np.append(int(val[2]),mat_dict[val[0]]['file'])
                mat_dict[val[0]]['burn']  = np.append(int(val[3]),mat_dict[val[0]]['burn'])
                mat_dict[val[0]]['uni']   = np.append(int(val[4]),mat_dict[val[0]]['uni'])
                mat_dict[val[0]]['bran']  = np.append(int(val[5]),mat_dict[val[0]]['bran'])
            k+=1+tot_branch
        if 'FILES' in line:
